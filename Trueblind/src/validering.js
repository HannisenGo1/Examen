export const validateFormData = (step, formData) => {
    let errors = {};
    if (step >= 1) {
    ...
    if (step >= 2) {
        if (!formData.gender)
            errors.gender = 'Kön är obligatoriskt.';
        if (!formData.sexualOrientation)
            errors.sexualOrientation = 'Sexuell läggning är obligatoriskt.';
    }
    if (step >= 3) {
        if (!formData.religion)
            errors.religion = 'Religion är obligatoriskt.';
        if (formData.interests.length < 5)
            errors.interests = 'Välj fem intressen.';
    }
    if (step >= 4) {
        if (formData.hasChildren === undefined)
            errors.hasChildren = 'Du måste svara på om du har barn.';
        if (formData.wantsChildren === undefined)
            errors.wantsChildren = 'Du måste svara på om du vill ha barn.';
        if (formData.smokes === undefined)
            errors.smokes = 'Du måste svara på om du röker.';
    }
    if (step >= 5) {
        if (!formData.relationshipStatus)
            errors.relationshipStatus = 'Vad söker du? är obligatoriskt.';
        if (!formData.education)
            errors.education = 'Utbildningsnivå är obligatoriskt.';
    }
    if (step >= 6) {
        if (!formData.favoriteSong)
            errors.favoriteSong = 'Favorit låt är obligatoriskt.';
        if (!formData.favoriteMovie)
            errors.favoriteMovie = 'Favorit film är obligatoriskt.';
    }
    return errors;
};
