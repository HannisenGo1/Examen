...
export const AccountPage = () => {
    ...
    {isOpen && (<div className="columndiv2">
      ...
      Ålder: {user?.age?.year && user?.age?.month && user?.age?.day
        ...
        {user.vipPlusStatus ? (<p>Du har <strong>VIP Plus</strong>, 
          {vipPlusDaysLeft > 0
                    ? `${vipPlusDaysLeft} dagar kvar på din VIP Plus.`
                    : 'Din VIP Plus har gått ut.'}
        </p>) : user.vipStatus ? (<p>Du har <strong>VIP</strong>, 
          {vipDaysLeft > 0
                    ? `${vipDaysLeft} dagar kvar på din VIP.`
                    : 'Din VIP har gått ut.'}
        </p>) : (<p>Du har ingen aktiv VIP.</p>)}
        
        
        <p>
        <strong>Stad:</strong>
        {isEditing.city ? (<input type="text" id="city" name="city" value={updatedUserData.city} onChange={handleChange} onBlur={() => handleBlur("city")}/>) : (<>
        ...
        {isEditing.sexualOrientation ? (<select id="sexualOrientation" name="sexualOrientation" value={updatedUserData.sexualOrientation} onChange={handleChange} onBlur={() => handleBlur("sexualOrientation")}>
        ...
        {showConfirm && (<div className="confirmation-box">
