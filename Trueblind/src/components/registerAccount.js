...
export const Register = () => {
    ...
    return (<>
        ...
        {isRegistered && (<div className="confirmation-message">
        ...
        {step === 1 && (<>
            ...
            <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} placeholder="Skriv din e-post"/>
            {errors.email && <span className="error">{errors.email}</span>}
            <label htmlFor="password">Lösenord </label>
            <input type="password" id="password" name="password" value={formData.password} onChange={handleChange} placeholder="Skriv in lösenord"/>
        ...
        <input type="number" id="month" name="month" value={formData.age.month} onChange={handleChange} min="1" max="12" placeholder="Välj månad"/>

        <label htmlFor="day">Dag</label>
        <input type="number" id="day" name="day" value={formData.age.day} onChange={handleChange} min="1" max="31" placeholder="Välj dag"/>

        <label htmlFor="year">År</label>
        <input type="number" id="year" name="year" value={formData.age.year} onChange={handleChange} min="1945" max="2025" placeholder="Skriv in år (4 siffror)"/>
        ...
        {step === 3 && (<>
            ...
            {intresseLista.map((interest) => (<button type="button" key={interest} className={`interest-button ${formData.interests.includes(interest.toLowerCase()) ? 'selected' : ''}`} onClick={() => handleInterestClick(interest.toLowerCase())} disabled={formData.interests.length >= 5 && !formData.interests.includes(interest.toLowerCase())}>
                {interest}
                </button>))}
            </div>
        ...
        {step === 4 && (<>
            ...
            Har du barn?
            <input type="checkbox" id="hasChildren" name="hasChildren" checked={formData.hasChildren} onChange={handleChange}/>
            </label>
            {errors.hasChildren && <span className="error">{errors.hasChildren}</span>}
            
            <label htmlFor="wantsChildren">
            Vill du ha barn?
            <input type="checkbox" id="wantsChildren" name="wantsChildren" checked={formData.wantsChildren} onChange={handleChange}/>
            </label>
            {errors.wantsChildren && <span className="error">{errors.wantsChildren}</span>}
            <label htmlFor="smokes">Röker du?</label>
            <div>
            <label>
            <input type="radio" id="smokes_no" name="smokes" value="nej" checked={formData.smokes === 'nej'} onChange={handleChange}/>
            Nej
            </label>
            <label>
            <input type="radio" id="smokes_yes" name="smokes" value="ja" checked={formData.smokes === 'ja'} onChange={handleChange}/>
            Ja
            </label>
            <label>
            <input type="radio" id="smokes_occasional" name="smokes" value="feströker" checked={formData.smokes === 'feströker'} onChange={handleChange}/>
            Feströker
            </label>
            </div>
        ...
        {step === 5 && (<>
            ...
            <select id="relationshipStatus" name="relationshipStatus" value={formData.relationshipStatus} onChange={handleChange}>
        ...
        {step === 6 && (<>
            ...
            <input type="text" id="favoriteSong" name="favoriteSong" value={formData.favoriteSong || ''} onChange={handleChange}/>
            
            <label htmlFor="favoriteMovie">Favorit film:</label>
            <input type="text" id="favoriteMovie" name="favoriteMovie" value={formData.favoriteMovie || ''} onChange={handleChange}/>
            <p> Avsluta meningen </p>
            <label>Jag skulle aldrig kunna leva utan...</label>
            <input type="text" name="lifeStatement1" value={formData.lifeStatement1} onChange={handleChange}/>
            
            <label>Jag blir mest inspirerad när...</label>
            <input type="text" name="lifeStatement2" value={formData.lifeStatement2} onChange={handleChange}/>
            
            
            <div className="register-terms-checkbox">
            <input type="checkbox" id="agreeTerms" checked={agreedToTerms} onChange={handleCheckboxChange}/>
        ...
        <ReCAPTCHA sitekey="6LcjQLcqAAAAANH5r9wv-Jaa3vQf6xlZVHJYsmjl" onChange={onChangeRe}/>
        </div>
     </>)}
        ...
        {step === 6 && <button className="accountBtn" type="submit" disabled={!(agreedToTerms && isReClicked)}>Registrera</button>}
        </div>
        
        </form> 
        {isTermsVisible && (<Anvandarpolicy onClose={handleCloseTerms} onAccept={handleAcceptTerms}/>)}
        </>);
};
