...
export const Chat = () => {
    ...
    const [otherUserName, setOtherUserName] = useState('Laddar...');
    ...
    const sendEmoji = async (emojiHtml) => {
    ...
    const handleEmojiClick = async (emojiName) => {
        ...
        if (emojiItem && emojiItem.count > 0) {
            ...
            const updatedEmojis = purchasedEmojis.map((emoji) => emoji.emoji === emojiBaseName
    ...
    <div className={openReport ? "report-container open" : "report-container"}>
      {!openReport ? (<button className="reportbtn" onClick={openTheReport}>Rapportera</button>) : (<>
    ...
    <div className="chat-container">
      <div className="chat-messages-container">
        {messages.length === 0 ? (<p>Inga meddelanden än.</p>) : (messages.map((msg) => (<div key={msg.id} className={`chat-message ${msg.senderId === user?.id ? 'sent' : 'received'}`}>
              <p className={`status-inlog ${currentChat?.status?.online ? 'online' : 'offline'}`}></p>
    ...
    <div className="chat-message-input-container">
      <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Skriv ditt meddelande..."/>
  ...
  {showEmojis && (<div className="emoji-picker">
      <div className="emoji-list">
        ...
        {Array.isArray(user.purchasedEmojis) && user.purchasedEmojis.length > 0 ? (user.purchasedEmojis.map((emoji) => {
                let emojiSrc;
                switch (emoji.emoji) {
                    case "nalle1":
                        emojiSrc = nalle1;
                ...
                return (<button key={emoji.emoji} onClick={() => handleEmojiClick(emojiSrc)} className={`emoji-item ${emojiClass(emoji.emoji)}`}>
