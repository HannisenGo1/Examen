...
export const Messages = () => {
    ...
    useEffect(() => {
        ...
        const uniqueRequests = Array.from(new Set(storedRequests.map((req) => req.senderId)))
            .map((senderId) => storedRequests.find((req) => req.senderId === senderId));
    ...
    {(requests && requests.length === 0) ? (<p>Inga nya meddelanden.</p>) : (requests && requests.filter(request => request.status === 'pending').map((request, index) => {
            const sender = likedUsers && likedUsers.find(user => user.id === request.senderId);
            if (!sender)
                return null;
            return (<div className="columndiv2" key={`${request.senderId}-${index}`}>
        ...
        <div className="columndiv2">
          <h2>Dina Chattar</h2>
          {activeChats.length === 0 ? (<p>Du har inga aktiva chattar.</p>) : (activeChats.map((chat) => {
            const userNames = chat.userNames?.filter(name => name !== user?.firstName) || [];
            return (<div className="divForallChats" key={chat.chatRoomId}>
