...
const VipPlusEmojis = ({ chatRoomId, sendEmoji }) => {
    ...
    const handleVipPlusEmojiClick = async (emojiSrc) => {
    ...
    {showEmojis && (<div className="emoji-list">
      {emojis.map((emoji) => (<button key={emoji.name} onClick={() => handleVipPlusEmojiClick(emoji.src)} className={`emoji-item ${emoji.className}`}>
