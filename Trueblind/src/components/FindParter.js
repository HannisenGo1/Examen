...
export const FindPartners = () => {
    ...
    const getUserStorageKey = (userId, key) => `${key}-${userId}`;
    ...
    const filterLikedAndDeniedUsers = (users, likedUsers, deniedUsers) => {
        return users.filter(user => !likedUsers.some(likedUser => likedUser.id === user.id) &&
    ...
    const [loadedDeniedUsers, setLoadedDeniedUsers] = useState([]);
    useEffect(() => {
        const storedDenied = JSON.parse(localStorage.getItem(getUserStorageKey(user?.id ?? "temp-user-id", "deniedUsers")) || "[]");
    ...
    return (<div className="columndiv3">
    <h2>Hitta din partner</h2>
    <label htmlFor="minAge">Ålder mellan:</label>
    <div className="age-filter-container">
    
    <input type="number" id="minAge" value={minAge} onChange={handleMinAgeChange} min="0" max="120"/>
    
    <input type="number" id="maxAge" value={maxAge} onChange={handleMaxAgeChange} min="0" max="120"/>
    </div>
    
    <div className="search-form">
    <label htmlFor="city">Stad:</label>
    <input type="text" id="city" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Exempel: Stockholm" className="input-field"/>
    
    <button onClick={filterUsers} disabled={loading} className="search-button">
    {loading ? 'Laddar...' : 'Sök partner'}
    </button>
    
    {isVip(user) && (<button onClick={restoreDeniedUsers} className="btn btn-primary">
    Återställ ❌
  </button>)}
    </div>
    
    {matchingResults.length > 0 && showCurrentUser ? (<div className="results-item">
      <ul className="results-list">
      <li key={showCurrentUser.id} className="result-item">
      <div className="result-info">
      <h4>
     
      {showCurrentUser.vipPlusStatus ? (<img src={viploggaplus} alt="viploggaplus" className="vip-logo"/>) : showCurrentUser.vipStatus ? (<img src={viplogga} alt="viplogga" className="vip-logo"/>) : null}
      </h4>
      ...
      Ålder: {user?.age?.year && user?.age?.month && user?.age?.day
                ? calculateAge(Number(user.age.year), Number(user.age.month), Number(user.age.day))
                : 'Ej angivet'}
        </strong>
        </p> </span></h4>
      ...
      </div>) : (<p className="no-results">Inga matchande resultat</p>)}
